...
| --- | --- | --- |
| 1. Baseline harness | Done as a library and offline CLI | Generative kinds still need a selected local backend |
...
| 10. CLI vertical slice | Partial | Fake conformance attaches; no started runtime |
...
6. `--diff`, `--dry-run`, `--trace`, escaped terminal rendering, `version`,
   `doctor`, fail-closed `rewrite`, and offline `rewrite-eval --baseline`
   exist. `rewrite --data-dir` inspects an active generation binding and
   `--artifact-id` must match it. A recovered binding whose qualification
   names the retained fake backend attaches in-process conformance and
   runs the grounded path. Any other backend still fails closed. Do not
   start a runtime or acquire a model. Generative baselines stay
   library-only until they share that attached conformance path.
