...
postpublication state failure leaves a retryable orphan. CLI exposure, downloads,
set inventory, reconciliation, removal, leases, runtime attestation, qualification,
activation, and execution remain later work.
...
`reconcile`, inactive `remove`, and exact `recover-removal`. It uses a
