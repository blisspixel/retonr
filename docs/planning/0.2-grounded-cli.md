...
| 10. CLI vertical slice | Partial | Completions, man pages, `model list`, and `model inspect` exist; fake conformance attaches; no started runtime |
...
6. `--diff`, `--dry-run`, `--trace`, escaped terminal rendering, `version`,
   ...
   `rewrite`, and offline `rewrite-eval --baseline` exist. `rewrite --data-dir`
