...
| 5. Native Ollama discovery | Partial with read-only, attached, and retained-connection preflights | Replace Linux proc TCP evidence, then complete canonical package, configuration, isolation, and runtime identity |
...
| 11. Model qualification | Not started | Freeze package identity and isolation before any generation |
| 12. Package and evidence | Partial | Model-backed screenshots after completion evidence |

Next implementation order:

1. Preserve the completed lifecycle, schema-v5 persistence, `import-set` CLI,
...
6. Replace Linux proc TCP row selection with bounded `NETLINK_SOCK_DIAG`, retaining
   exact tuple, socket cookie, inode, UID, and namespace evidence. This removes the
   deprecated proc TCP table from the Linux row-selection boundary before stronger
   runtime claims depend on it.
7. Reconstruct the selected external Ollama package as one canonical artifact-set
   manifest with complete blob, tokenizer, template, license, upstream source, and
   transformation evidence. Keep runtime-build identity separate from model-package
   identity.
8. Require version-gated provider cloud-disable evidence and OS-enforced
   non-loopback denial for every Retonr and runtime process before a local generation
   plan becomes eligible.
9. Construct effective runtime identity only after the transport, canonical package,
   provider configuration, and OS isolation evidence join without drift. Then
   project the existing eight-case smoke and 39-case editorial corpus into
   versioned local generation plans with exact request, resource, cancellation,
   residency, and result contracts. The previous Gemma 4, Qwen3.6, and Ministral
   local observations have expired. Do not reacquire a model without separate
   approval.
10. Run smoke, locked evaluation, repeatability, and cross-platform qualification in
   that order. The existing hard gates remain the only acceptance authority.
11. Finish remaining CLI recovery and packaging evidence, then capture model-backed
...
The loopback-only adapter, fake conformance coverage, and versioned live read-only
preflight are implemented. Preflight brackets model inspection with runtime,
inventory, and residency observations, exposes no Retonr artifact identity, performs
no generation, and always reports unqualified. Remaining work starts by binding the
listener to exact process and runtime-build evidence, then completing package and
isolation evidence before any generative qualification run.
