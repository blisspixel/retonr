...
set inventory, reconciliation, removal, runtime attestation, qualification,
...
exclusive repository operation fails while a lease is live. Managed-set removal does
not exist yet, so a set installation generation is always the first generation and
cannot yet distinguish a reinstall.
