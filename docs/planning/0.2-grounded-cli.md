...
| 2. Manifests and lifecycle | Done for offline lifecycle | No downloads |
| 3. Inference contracts | Done with claim schema | Pair extraction exists; no engine authority |
| 4. Fake HTTP backend | Done for Ollama | Shared conformance server is optional |
| 5. Native Ollama discovery | Partial against fakes | Managed-process attestor exists; do not treat observed Ollama as a build identity |
...
| 10. CLI vertical slice | Partial | Completions, man pages, source `inspect` including bounded recursive directory discovery, `--in-place` for one regular file, `rewrite` `--diff`/`--dry-run`/`--trace`, short flags, `model list`, `model inspect`, optional fitr `device-evidence`, and actionable `doctor` recovery exist; fake conformance attaches; no directory rewrite; no started runtime |
| 11. Model qualification | Not started | Not $0; needs already-installed artifacts after joins |
| 12. Package and evidence | Partial | Model-backed screenshots after completion evidence |

Next implementation order:

1. Preserve the completed lifecycle, `import-set` CLI, inventory DTOs, and
   cancellation evidence.
2. The managed-process attestor is implemented. Keep it inert: no role, no
   observed-only Ollama build identity, no claim-extraction enablement.
3. The extractor manifest and claim-output schema exist. Ollama still admits
   only the candidate contract.
4. The cancellable pair-extraction service exists and compares independently.
   It has no activation authority.
5. The application can join completed pair-extraction comparison onto the
   engine's informational shadow gate. `rewrite-eval --claim-shadow-calibration`
   runs fixture-assigned identities independently and fails if shadow
   evidence changes acceptance. Literal-token failure still abstains.
   A claim conflict cannot reject a candidate that passed the hard gates.
   Ollama still admits only the candidate contract.
6. `--diff`, `--dry-run`, `--trace`, escaped terminal rendering, `version`,
   `doctor`, `completions`, `man`, source `inspect`, `model list`,
   `model inspect`, fail-closed
   `rewrite`, and offline `rewrite-eval --baseline` exist. `inspect` reports
   encoding, BOM, controls, sidecar presence, and a directory discovery
   manifest. `--recursive` is bounded and does not follow links.
   `--in-place` replaces one regular file after a sibling backup.
   It is not a directory rewrite. `doctor` reports
   pending-removal counts and names `model.migrate` or removal recovery when
   those operations are required. It does not mutate. `rewrite --data-dir`
   inspects an active generation binding and `--artifact-id` must match it.
   A recovered binding whose qualification names the retained fake backend
   attaches in-process conformance and runs the grounded path.
   `rewrite-eval --baseline --data-dir` uses that same recovered binding for
   generative kinds. Any other backend still fails closed. `model list` and
   `model inspect` do not qualify or activate. `rewrite` shares `--diff`,
   `--dry-run`, and `--trace` with `check`. Optional `model device-evidence`
   reads fitr measurement JSON without a repository and is not a
   qualification. Do not start a runtime or acquire a
   model.
7. Preserve read-only set inventory, selected set reconciliation, and
...
`model remove-set` with exact `recover-set-removal`. Downloads, runtime
attestation, qualification, activation, and execution remain later work.
...
only the existing candidate schema. Claim extraction remains a separate
planned role and application operation with no product authority in this slice.
