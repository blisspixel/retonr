...
| 5. Native Ollama discovery | Partial with fake conformance and read-only live preflight | Bind an attached listener to exact runtime-build evidence |
...
4. Extend the managed-process attestation boundary to attach the loopback listener
   to the exact user-managed Ollama process, regular entrypoint, runtime version,
   launch mode, and effective configuration before and after use.
5. Reconstruct the selected external Ollama package as one canonical artifact-set
   ...
   identity.
6. Require explicit cloud disablement and OS-enforced non-loopback denial for every
   Retonr and runtime process before a local generation plan becomes eligible.
7. Project the existing eight-case smoke and 39-case editorial corpus into
   ...
   approval.
8. Run smoke, locked evaluation, repeatability, and cross-platform qualification in
