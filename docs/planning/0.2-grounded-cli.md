...
| 10. CLI vertical slice | Partial | Completions, man pages, source `inspect` including bounded recursive directory discovery, `--in-place --backup` for one regular file, `model list`, `model inspect`, and actionable `doctor` recovery exist; fake conformance attaches; no directory rewrite; no started runtime |
...
6. `--diff`, `--dry-run`, `--trace`, escaped terminal rendering, `version`,
   ...
   `--in-place --backup` replaces one regular file after a sibling backup.
   ...
   `model inspect` do not qualify or activate. Do not start a runtime or acquire a
   model.
