...
read-only `pending-operations`, selected `reconcile`, inactive `remove`, and exact
`recover-removal`. It uses a
...
- Retain the implemented offline import, inventory, pending-operations, reconcile,
  remove, and recover-removal commands, then add model list, inspect, explicit
...
| Artifact lifecycle | Runtime-pull versus product-download mode, approved sources, TLS, proxy and redirect policy, explicit network consent, bytes and disk, offline import, resume integrity, digest and size, staging, selected exact reconciliation target, stale inventory, absent or conflicting target, indirect and aliased entries, entry and byte ceilings, cancellation, process interrupts, bounded pending-operation inspection, state conflict, idempotent retry, no activation, license, activation, in-use removal, cleanup, and deletion |
