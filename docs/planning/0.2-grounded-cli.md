...
| 10. CLI vertical slice | Partial | Completions, man pages, source `inspect`, `model list`, `model inspect`, and actionable `doctor` recovery exist; fake conformance attaches; no started runtime |
...
6. `--diff`, `--dry-run`, `--trace`, escaped terminal rendering, `version`,
   ...
   encoding, BOM, controls, and sidecar presence without credential
   validation. `doctor` reports
