...
| 5. Native Ollama discovery | Partial with read-only preflight and attached-process witness | Bind one retained HTTP connection to its accepted server socket, then complete runtime-build evidence |
...
5. Own one persistent HTTP connection and bind its exact client and server 4-tuple to
   the accepted server socket held by the retained process. Reject reconnects,
   migration, handoff, ambiguity, or incomplete visibility.
6. Reconstruct the selected external Ollama package as one canonical artifact-set
   ...
   identity.
7. Require explicit cloud disablement and OS-enforced non-loopback denial for every
   Retonr and runtime process before a local generation plan becomes eligible.
8. Project the existing eight-case smoke and 39-case editorial corpus into
   ...
   approval.
9. Run smoke, locked evaluation, repeatability, and cross-platform qualification in
   that order. The existing hard gates remain the only acceptance authority.
10. Finish remaining CLI recovery and packaging evidence, then capture model-backed
