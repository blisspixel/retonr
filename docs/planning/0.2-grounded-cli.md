...
| 10. CLI vertical slice | Partial | Completions, man pages, source `inspect` including bounded recursive directory discovery, `model list`, `model inspect`, and actionable `doctor` recovery exist; fake conformance attaches; no started runtime |
...
6. `--diff`, `--dry-run`, `--trace`, escaped terminal rendering, `version`,
   ...
   manifest. `--recursive` is bounded and does not follow links. `doctor` reports
