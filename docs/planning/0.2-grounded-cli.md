...
Status: planned. Implementation may begin only after the remaining 0.1 evidence is
closed.
...
The current repository has the deterministic transaction, candidate-check CLI, and
hard-negative harness. The following 0.1 items must be completed and evidenced
before 0.2 is treated as active:

1. Add no-rewrite, direct-prompt, style-description, and retrieved-example baseline
   runner contracts.
2. Add enough positive paraphrase fixtures to measure transformation coverage, not
   only rejection behavior.
3. Record the authorized-data and deletion policy for any non-synthetic evaluation.
4. Record the initial user-research and adjudication protocol.
5. Run the existing deterministic transaction on Windows, macOS, and Linux in remote
   continuous integration and retain the results.
6. Complete the four 0.1 refinement passes and record any accepted exceptions.
...
rewrite-eval depends on public application ports and qualification records.
Backend adapters depend on inference contracts, not engine internals.
```
...
are available. For every artifact and quantization, record:
...
- Run clean-install, missing-runtime, missing-model, offline, file-lock, long-path,
  non-UTF-8 path where supported, pipe, redirection, and cancellation tests.
...
- Network access is denied under local-only mode, including redirects and alternate
...
| Network | Socket-denial test proving qualified local-only rewrites need no external connection |
...
- Local-only enforcement has automated no-network evidence.
