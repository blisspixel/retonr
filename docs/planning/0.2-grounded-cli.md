...
adapter conformance. The following items remain before 0.2 is treated as active:

1. Approve or revise the proposed governance records. No non-synthetic collection is
   authorized before that decision.
2. Run the final 0.1 closure revision through the complete retained matrix and record
   the exact evidence.
3. Complete the four-pass refinement disposition with no undispositioned critical
   defect or implicit exception.
4. Review the proposed decision records listed below at the 0.2 entry gate.
