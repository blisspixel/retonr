...
| 10. CLI vertical slice | Partial | Completions, man pages, `model list`, `model inspect`, and actionable `doctor` recovery exist; fake conformance attaches; no started runtime |
...
6. `--diff`, `--dry-run`, `--trace`, escaped terminal rendering, `version`,
   `doctor`, `completions`, `man`, `model list`, `model inspect`, fail-closed
   `rewrite`, and offline `rewrite-eval --baseline` exist. `doctor` reports
   ...
   generative kinds. Any other backend still fails closed. `list` and
   `inspect` do not qualify or activate. Do not start a runtime or acquire a
   model.
