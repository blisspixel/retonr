...
Status: planned. Technical 0.1 implementation evidence is complete. Implementation
may begin only after the remaining owner decisions are recorded.
...
adapter conformance. Exact-main quality run `31658435581` passed for implementation
closure revision `b2d41fc`. The following owner decisions remain before 0.2 is
treated as active:

1. Accept or revise ADR 0001 and the completed four-pass refinement disposition to
   close milestone 0.1.
2. Approve or revise the proposed governance records. No non-synthetic collection is
   authorized by owner approval alone; every additional policy precondition remains
   mandatory.
3. Accept or revise ADRs 0003 through 0005 at the 0.2 entry gate.

The exact commitments and non-authorizations are consolidated in the
