...
| 7. Grounded strategy | Done as a library | CLI `rewrite` exists; no selected artifact |
...
| 10. CLI vertical slice | Partial | Repository binding inspect exists; no attached runtime |
...
6. `--diff`, `--dry-run`, `--trace`, escaped terminal rendering, `version`,
   ...
   `--artifact-id` must match it. A recovered binding still fails closed
   because no local runtime is attached. Do not start a runtime or acquire a
   model. Generative baselines stay library-only until that runtime exists.
