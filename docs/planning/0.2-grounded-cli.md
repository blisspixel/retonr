...
| 10. CLI vertical slice | Partial | Completions and man pages exist; fake conformance attaches; no started runtime |
...
6. `--diff`, `--dry-run`, `--trace`, escaped terminal rendering, `version`,
   `doctor`, `completions`, `man`, fail-closed `rewrite`, and offline
   `rewrite-eval --baseline` exist. `rewrite --data-dir` inspects an active
   generation binding and `--artifact-id` must match it. A recovered binding
   whose qualification names the retained fake backend attaches in-process
   conformance and runs the grounded path. `rewrite-eval --baseline --data-dir`
   uses that same recovered binding for generative kinds. Any other backend
   still fails closed. Do not start a runtime or acquire a model.
