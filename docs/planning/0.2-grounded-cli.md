...
| 10. CLI vertical slice | Partial | Completions, man pages, source `inspect` including non-recursive directory discovery, `model list`, `model inspect`, and actionable `doctor` recovery exist; fake conformance attaches; no started runtime |
...
6. `--diff`, `--dry-run`, `--trace`, escaped terminal rendering, `version`,
   ...
   encoding, BOM, controls, sidecar presence, and a non-recursive directory
   discovery manifest. It does not follow links or recurse. `doctor` reports
