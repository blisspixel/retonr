...
6. `--diff`, `--dry-run`, `--trace`, escaped terminal rendering, `version`,
   ...
   runs the grounded path. `rewrite-eval --baseline --data-dir` uses that
   same recovered binding for generative kinds. Any other backend still
   fails closed. Do not start a runtime or acquire a model.
