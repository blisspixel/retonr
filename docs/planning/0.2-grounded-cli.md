...
| 2. Manifests and lifecycle | Done for offline lifecycle plus inert runtime/model package import and persistence, including reviewed Linux Ollama runtime-package reconstruction | No downloads or authority |
...
| 5. Native Ollama discovery | Partial with read-only, attached, retained-connection, package, cloud-disable, Linux managed-isolation, inert managed preflight and runtime-build binding, static v0.32.15 model binding, retained-session judge executor, residency receipt, and admission-gated one-shot retained managed generation | Admit one runtime, add the four remaining direct effective-state relationships, and join model-package lease plus judge evidence |
...
| 11. Model qualification | Exact Linux CPU candidate review active and not admitted | Resolve source-lineage and external-component blockers, run managed startup and cloud-disable controls, keep the cloud allowlist empty until then, and qualify only after receipts |
...
7. Retain complete runtime-package, model-package, native-load, static package-lease,
   ...
   and managed import produce only inert model-package evidence. The offline reviewed
   Linux Ollama runtime-package reconstruction and managed import produce only inert
   runtime-package evidence from a caller-supplied layout and exact member bytes;
   extra tree files fail closed, the isolation helper is not executed, and the
   production cloud-disable allowlist remains empty. Retain the typed exact-review
   contract and the checked-in v0.32.15 Linux CPU x64 non-admitted disposition. Its
   source archive, complete transformation accounting, helper, selected native
   closure, and license inventory are frozen, while its unresolved and unrun controls
   remain explicit. Retain the exact
...
11. Retain the admission-gated one-shot managed generation operation. The current
    empty reviewed-runtime allowlist blocks it before launch. Once admitted, it keeps
    the process, runtime package lease, native observer, and direct connection live
    through one structured
    completion, binds the static model relationship and distinct artifact and
    inventory digests, records equal post-generation residency and effective context,
    then closes the process tree. It remains inert and has no CLI surface. Complete
    the frozen v0.32.15 Linux CPU x64 review: resolve source lineage and
    exact external platform-component identity, then run managed startup and
    cloud-disable observation. Only an all-pass typed disposition may carry the final
    layout and package identities into the currently empty production cloud-disable
    allowlist. Add the generation-bound provider, effective output configuration,
    platform and driver, and compute-placement evidence still required for effective
    state. Join the exact model-package lease and judge receipt. Then add a distinct
...
runtime-reported residency receipt, the Linux-only managed preflight and
runtime-build binding, and one admission-gated one-shot retained managed generation
bracket are implemented. The empty reviewed-runtime allowlist blocks that bracket
before launch. Attached and bound reports remain
...
runtime trust observation is unsupported. The one-shot operation keeps one process,
runtime package lease, native observer, and direct connection through generation,
joins the static model binding, preserves distinct model artifact and inventory
identity, and records effective context plus runtime-reported residency before
cleanup. It does not retain an exact model-package lease or construct effective state.
Remaining work is one admitted runtime package, four direct effective-state
relationships, model-package and judge joins, a candidate receipt, then qualification.
