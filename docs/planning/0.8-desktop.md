...
- CLI rewrite, profile, model, Markdown, DOCX, batch, recovery, and report workflows
  pass their cross-platform gates.
...
- The exact supported operating-system, architecture, graphics, input, package, and
  minimum-hardware matrix is approved.
- Comparable native Rust UI spikes have been run on every target platform.
- Accessibility requirements and named manual test procedures are approved before a
  toolkit is selected.
