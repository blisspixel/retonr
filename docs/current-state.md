...
| `rewrite-ollama-package` | Strict bounded parsing of the admitted Ollama manifest-v2 layer shape and GGUF v3 metadata and tensor table, followed by deterministic reconstruction of one canonical six-member model artifact set and semantic model-package manifest; bounded reconstruction of one reviewed Linux x86_64 GNU libc Ollama runtime package from a layout and exact member bytes, including exactly one generation worker, an isolation helper that must not be code-loaded, and an exact transformation record whenever the package differs from its source artifact set; plus a typed fail-closed exact package-review disposition whose admission form requires every required control to pass and whose non-admitted form must list every blocked or unrun control; no network, mutable tag authority, qualification, activation, load, or execution |
...
- Durable artifact lifecycle state, bounded single-file staging recovery, pinned single-file
  ...
  one transformation-record member. Neither import has a CLI surface. Neither
  qualifies, activates,
...
1. Preserve schema-6 lifecycle, migration, retained package-object, SOCK_DIAG,
   ...
   boundaries.
2. Freeze and review one complete Ollama runtime package. Generic reviewed-layout
   reconstruction exists. The exact v0.32.15 Linux CPU x64 candidate now freezes the
   official archive, complete 51-entry source accounting, one baseline CPU closure,
   the Retonr helper, and all selected-code license dispositions. Its typed review is
   not admitted: upstream source-to-binary lineage and exact external platform
