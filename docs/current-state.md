...
- Durable artifact lifecycle state, bounded single-file staging recovery, pinned single-file
  ...
  requires one isolation helper, stored as helper-executable evidence that must not be
  code-loaded. Neither import has a CLI surface, and neither qualifies, activates,
...
2. Freeze and review one complete Ollama runtime package. Generic reviewed-layout
   reconstruction exists, but one exact upstream package still needs complete helper,
   native-dependency, source, transformation, and license disposition. Add an exact
   runtime to the production cloud-disable allowlist only after that review passes.
