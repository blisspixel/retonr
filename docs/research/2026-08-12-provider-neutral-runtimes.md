...
Retonr should qualify two local generation paths as the first supported runtime
catalog entries:
...
Experimental adapters for other current major runtimes may proceed in parallel when
they reuse the same inference port and conformance suite. They remain experimental
until their runtime-specific identity and qualification evidence passes.

LM Studio, vLLM, and MLX LM are useful 0.x experimental candidates.
